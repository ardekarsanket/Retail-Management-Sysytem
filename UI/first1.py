...
"padding-bottom:7px;")
        ...
        self.pushButton_2.clicked.connect(self.second)                ##4
        ...
        self.pushButton_3.clicked.connect(Form.close)                                    ##3
    ...
    def DBConnect(self):
        try:
                db = mdb.connect('localhost', 'sanket', '12345678', 'rms')
                #self.messagebox("Congrats", "DB Connected")
                mycursor=db.cursor()
                user = self.lineEdit.text()
                pass1 = self.lineEdit_2.text()
                query = "SELECT username,password from admin WHERE username like'"+user+"'and password like'"+pass1+"' "
                mycursor.execute(query)
                result = mycursor.fetchone()
                
                if result == None:
                        self.messagebox("Invalid", "Invalid input")
                else:
                        
                        self.post_admin_login(x)
                        
                        
        except mdb.Error as e:
                self.messagebox("DB Error", "Connection Failed")
                #sys.exit(1)
'''
    def passage(self):
    ...
    def admin_logincheck(self,x):
        ...
        if(usern != None and pass11 != None):
                ...
                if((usern == None) and (pass11 == None) and (check == False)):
                        self.messagebox("Input Error","Fields are Empty :(")
                elif((usern == None) and (pass11 == None) and (check == True)):
                        self.messagebox("Input Error","Invalid username and password :(")
                elif(usern == None):
                        self.messagebox("Input Error","Invalid username! Try Again!")
                else:
                        self.messagebox("Input Error", "Invalid password! Try again!")    
        connection.close()            
